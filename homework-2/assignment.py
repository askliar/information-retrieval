...
collection_frequencies = dict(collection_frequencies)
...
def run_retrieval(model_name, score_fn):
    ...
    for query_id in tokenized_queries:
        ...
        for int_doc_id in range(index.document_base(), index.maximum_document()):
            ...
            if doc_score > 0:
                print('appendind ', (doc_score, ext_doc_id))
                data[query_id].append((doc_score, ext_doc_id))
            print('end of doc', int_doc_id)
        print('end of query', query_id)

                #         # These are the usual ipython objects, including this one you are creating
                #         ipython_vars = ['In', 'Out', 'exit', 'quit', 'get_ipython', 'ipython_vars']

                #         # Get a sorted list of the objects and their sizes
                #         a = sorted([(x, sys.getsizeof(globals().get(x))) for x in dir() if not x.startswith('_') and x not in sys.modules and x not in ipython_vars], key=lambda x: x[1], reverse=True)
                #         print(a)
...
def tfidf(int_document_id, query_id):
    ...
    for query_term_id in tokenized_queries[query_id]:
        ...
        if int_document_id in inverted_index[query_term_id]:
            ...
            idf = num_documents / df
            score += np.log(1 + tf) * np.log(idf)
            # print('calculated score for ', query_term_id, int_document_id)
    # print('fine', query_term_id, int_document_id)
    return score
...
# TODO implement tools to help you with the analysis of the results.
        ...
        #         if int_document_id in inverted_index[query_term_id]:
        #             document_term_freq = inverted_index[query_term_id][int_document_id]
        #             # print(type(document_term_freq), document_term_freq)
        #             tf = document_term_freq
        #             df = len(inverted_index[query_term_id])
        #             idf = num_documents/df
        ...
        #
        #     print('i dont know da wei i only know aiml')
        #     return score
        # # objgraph.show_most_common_types(limit=20)
        # run_retrieval('tfidf', tfidf)
        # # obj = objgraph.by_type('list')
...
#             second_term = np.exp( (np.log(k1+1)+np.log(tf_td)) - np.exp( (np.log(k1) + np.log((1-b)+np.log(b)+np.log(Ld) - np.log(avg_doc_length)))+tf_td))
...
#         first_term = np.exp(np.log(lambd) + np.log(tf) - np.log(doc_len))
...
#     kernel_val = np.exp(-np.power(i-j, 2)/(2*np.power(sigma,2)))
#     return kernel_val
#
# def gauss_kernel_integral(ld, rd, sigma=50):
#     partial_integral = lambda i: np.sqrt(2*np.pi*np.power(sigma,2)) * (norm.cdf(i/sigma) - norm.cdf(0))
#     return partial_integral(ld) + partial_integral(rd)
#
# def triangle_kernel(i, j, sigma=50):
#     kernel_val = 1-np.abs(i-j)/sigma if np.abs(i-j)<=sigma else 0
#     return kernel_val
#
# def triangle_kernel_integral(ld, rd, sigma=50):
#     partial_integral = lambda i: i - np.power(i, 2)/(2*sigma) if i < sigma else sigma/2
#     return partial_integral(ld) + partial_integral(rd)
#
...
#     return kernel_val
#
# def cosine_kernel_integral(ld, rd, sigma=50):
#     partial_integral = lambda i: sigma/2 + sigma*np.sin(np.pi*i/sigma)/(2*np.pi) if i < sigma else sigma/2
#     return partial_integral(ld) + partial_integral(rd)
#
# def circle_kernel(i, j, sigma=50):
#     kernel_val = np.sqrt(1-np.power(np.abs(i-j)/sigma,2))        if np.abs(i - j)<=sigma        else 0
#     return kernel_val
#
# def circle_kernel_integral(ld, rd, sigma=50):
#     partial_integral = lambda i: sigma/2 * (np.arcsin(i/sigma) + 1/2 * np.sin(2*np.arcsin(i/sigma)))                        if i < sigma else sigma*np.pi/4
#     return partial_integral(ld) + partial_integral(rd)
#
...
#     return kernel_val
#
# def passage_kernel_integral(ld, rd, sigma=50):
#     partial_integral = lambda i: i if i < sigma else sigma
#     return partial_integral(ld) + partial_integral(rd)
#
#
# # In[ ]:
...
#                                      (circle_kernel, circle_kernel_integral),
#                                      (passage_kernel, passage_kernel_integral)]:
#     plm_func = functools.partial(positional_language_model, kernel=kernel_func, kernel_integral=kernel_integral)
#     run_retrieval('Dirichlet smoothed PLM', plm_func)
#
#
#
...
#
#
# print(index.document(329))
# print(tokenized_queries['51'])
# for i in tokenized_queries['51']:
#     if 329 in inverted_index_positions[i]:
#         print(inverted_index_positions[i][329])
#
#
