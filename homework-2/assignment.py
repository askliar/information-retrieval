...
def gauss_kernel_integral(ld, rd, sigma=50):
    partial_integral = lambda i: np.sqrt(2*np.pi*np.pow(sigma,2)) * (norm.cdf(i/sigma) - norm.cdf(0))
...
def triangle_kernel_integral(ld, rd, sigma=50):
    partial_integral = lambda i: i - np.pow(i, 2)/(2*sigma) if i < sigma else sigma/2
    return partial_integral(ld) + partial_integral(rd)
...
def cosine_kernel_integral(ld, rd, sigma=50):
    partial_integral = lambda i: sigma/2 + sigma*np.sin(pi*i/sigma)/(2*np.pi) if i < sigma else sigma/2
    return partial_integral(ld) + partial_integral(rd)
...
def circle_kernel_integral(ld, rd, sigma=50):
    partial_integral = lambda i: sigma/2 * (np.arcsin(i/sigma) + 1/2 * sin(2*np.arcsin(i/sigma)))                        if i < sigma else sigma*np.pi/4
    return partial_integral(ld) + partial_integral(rd)
